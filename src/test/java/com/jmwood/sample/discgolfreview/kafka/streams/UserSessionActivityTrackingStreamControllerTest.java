...
public class UserSessionActivityTrackingStreamControllerTest {
    ...
    private SessionActivityRepository mockSessionActivityRepository = Mockito.mock(SessionActivityRepository.class);

    @Before
    public void setup() {
        StreamConfiguration streamConfiguration = new StreamConfiguration();
        KafkaStreamsConfiguration kafkaStreamsConfiguration = streamConfiguration.streamsConfiguration(
                "testStream",
                "sampleServer:1111");

        UserSessionActivityTrackingStreamController streamController = new UserSessionActivityTrackingStreamController(
        ...
        authEventTestInputTopic = topologyTestDriver.createInputTopic(TEST_AUTH_EVENT_TOPIC, Serdes.String().serializer(), eventSerializer);
        navEventTestInputTopic = topologyTestDriver.createInputTopic(TEST_NAV_EVENT_TOPIC, Serdes.String().serializer(), eventSerializer);
        courseEventTestInputTopic = topologyTestDriver.createInputTopic(TEST_COURSE_EVENT_TOPIC, Serdes.String().serializer(), eventSerializer);
        clickEventTestInputTopic = topologyTestDriver.createInputTopic(TEST_CLICK_EVENT_TOPIC, Serdes.String().serializer(), eventSerializer);

        sessionActivityTestOutputTopic = topologyTestDriver.createOutputTopic(TEST_SESSION_ACTIVITY_TOPIC, Serdes.String().deserializer(), StreamConfiguration.getJsonSerdeForClass(SessionActivity.class).deserializer());
    }
    ...
    public void verifyEventAppersInOutputTopic() {
        ...
        //then
        SessionActivity expectedSessionActivity = new SessionActivity(sessionId, user, List.of(testAuthEvent));

        TestRecord<String, SessionActivity> actualSessionActivityRecord = sessionActivityTestOutputTopic.readRecord();
    ...
    public void verifyEventsCombineIntoSingleSessionActivityAndVerifyEachUpdate() {
        ...
        //then
        SessionActivity expectedSessionActivity = new SessionActivity(sessionId, user, new ArrayList<>(List.of(testAuthEvent)));
    ...
    public void verifyConcurrentSessionsCreateDifferentSessionActivityObjects() {
        ...
        //when
        authEventTestInputTopic.pipeRecordList(List.of(new TestRecord<String, Event>(sessionId_1, testAuthEvent_1), new TestRecord<String, Event>(sessionId_2, testAuthEvent_2)));
        navEventTestInputTopic.pipeRecordList(List.of(new TestRecord<String, Event>(sessionId_2, testNavEvent_2), new TestRecord<String, Event>(sessionId_1, testNavEvent_1)));

        //then
        SessionActivity expectedSessionActivity_1 = new SessionActivity(sessionId_1, user_1, List.of(testAuthEvent_1, testNavEvent_1));
        SessionActivity expectedSessionActivity_2 = new SessionActivity(sessionId_2, user_2, List.of(testAuthEvent_2, testNavEvent_2));

        Map<String, SessionActivity> actualSessionActivityMap = sessionActivityTestOutputTopic.readKeyValuesToMap();
        assertEquals(expectedSessionActivity_1, actualSessionActivityMap.get(expectedSessionActivity_1.getId()));
        assertEquals(expectedSessionActivity_2, actualSessionActivityMap.get(expectedSessionActivity_2.getId()));
    ...
    public void verifyAllEventTypesCombineToSingleSessionActivity() {
        ...
        //then
        SessionActivity expectedSessionActivity = new SessionActivity(sessionId, user, List.of(testAuthEvent, testNavEvent, testCourseEvent, testClickEvent));

        Map<String, SessionActivity> actualSessionActivityMap = sessionActivityTestOutputTopic.readKeyValuesToMap();
        SessionActivity actualSessionActivity = actualSessionActivityMap.get(expectedSessionActivity.getId());
